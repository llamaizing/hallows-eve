...
function get_unique_speakers(conversation)
  unique_speakers = {}
  for _k, speaker in ipairs(conversation) do
    is_unique = true
    for _k, unique in ipairs(unique_speakers) do
...
function conversation_parser(dialog, info)
  conversation_text = substitutions(dialog, info)

  speaker_list = { id = dialog.id }
  ...
  for speaker_lines in string.gmatch(conversation_text, "@.-===") do
    speaker = parse_options(speaker_lines)
...
function parse_name(lines)
  name_matcher = '^@(.+)$'
  name = string.match(lines[1], name_matcher) -- Name should only be on the first line
...
--
-- Returns the name and the rmainder of the string
function parse_inline_options(lines)
  options = {}
  options['inline_options'] = {}
  parsed_lines = {}
  for index, line in pairs(lines) do
    for flag_and_value in string.gmatch(line, '%b$$') do
      flag, value = string.match(flag_and_value, '%$(%S+)=(%S+)%$')

      flag_first, val_last = string.find(line, flag_and_value, 1, true) -- by default string.find matches patterns and $ is a special character. true option forces find by plain string
...
function remove_flag_info_from_line(flag_first, val_last, line)
  modified_line = ""
...
--
-- Example:
--   parse_dialog('When I joined the corps, we didn't have any fancy shmancy tanks.\n We had sticks! Two sticks and a rock for the whole platoon, \n and we had to share the rock! \n Buck up, you're one lucky marine! ===')
--     {[ 
...
function parse_dialog(dialog)
  ...
  first, _last = dialog:find("%S")
  cleaned_lines = dialog:sub(first) -- Each line including empty ones. 
...
function substitutions(dialog, info)
